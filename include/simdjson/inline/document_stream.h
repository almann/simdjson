 ...
 * */
inline uint32_t find_last_json_buf_idx(const uint8_t *buf, const dom::parser &parser) {
...
inline error_code document_stream::json_parse() noexcept {
  ...
  if (unlikely(load_next_batch)) {
    ...
    if (!stage_1_thread.joinable()) {
      ...
      uint32_t last_index = internal::find_last_json_buf_idx(buf(), parser);
      if (last_index == 0) {
        if (parser.n_structural_indexes == 0) {
          return simdjson::EMPTY;
        }
      } else {
        parser.n_structural_indexes = last_index;
      }
    }
    ...
    if (remaining() - _batch_size > 0) {
      last_json_buffer_loc =
          parser.structural_indexes[internal::find_last_json_buf_idx(buf(), parser)];
